...
{
    /// <summary>
    /// Resize image extension to return a Nearest Neighbor scaled image
    /// </summary>
    ...
    {
        if (!image.IsValid) throw new ArgumentException($"{nameof(ResizeHalf)}: {nameof(image)} is NOT valid.");
    ...
    {
        if (image.Channels != 1) throw new ArgumentException($"{nameof(ResizeHalfGray)}: {nameof(image)} unsupported number of channels.");
    ...
    {
        if (!image.IsValid) throw new ArgumentException($"{nameof(Resize)}: {nameof(image)} is NOT valid.");
    ...
    {
        if (image.Channels != 1) throw new ArgumentException($"{nameof(ResizeGray)}: {nameof(image)} unsupported number of channels.");
